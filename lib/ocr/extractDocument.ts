import type { ExtractedCell, ExtractedRow } from "@/lib/types";
...
async function mockExtractDocument(input: ExtractDocumentInput): Promise<ExtractedRow[]> {
  ...
  return [
    ...
    },
    {
      ...
      lossesAndAdjustments: cell("O", 0.48, "OCR read a letter where a number is expected."),
    ...
    },
    {
      ...
      quantityReceived: cell("", 0.44, "Blank or unreadable quantity."),
...
async function providerExtractDocument(input: ExtractDocumentInput): Promise<ExtractedRow[]> {
  if (!process.env.OCR_PROVIDER_API_KEY) return mockExtractDocument(input);

  // Replace this adapter with the chosen OCR/AI provider call. Keep the return
  // contract as ExtractedRow[] so validation and UI behavior remain unchanged.
  return mockExtractDocument(input);
}

export async function extractDocument(input: ExtractDocumentInput) {
  const rawRows = await providerExtractDocument(input);
  return validateExtractedRows(normalizeExtractedRows(rawRows));
}
